...
# os.environ['WORKING_DIR_IMPORT_MODE'] = 'train_movie'  # Change this to your target mode
# os.environ['WORKING_DIR_IMPORT_MODE'] = 'train_movie'  # Change this to your target mode
# os.environ['WORKING_DIR_IMPORT_MODE'] = 'eval_movie'  # Change this to your target mode

Time_log = False
import csv
from datetime import datetime
# os.environ['WORKING_DIR_IMPORT_MODE'] = 'eval_miccai30fps'  # Change this to your target mode
...
from torch.autograd import Variable
import eval_slots
from eval import get_model_infer_flops,get_component_flops
...
# Output_root = Output_root+ "Obj_centric_temp2MLP_BERT" + selected_data + str(Data_percentage) + "/"
# io.self_check_path_create(Output_root)
# Contrastive_temp = 2.2
# Contrastive_temp = 0.01
# Contrastive_temp = 2.0
# Contrastive_temp = 1.5
# Contrastive_temp = 10
...
Context_len = 11
# Contrastive_temp = 100.0
# Contrastive_temp = 0.001




dataset_tag = "+".join(selected_data) if isinstance(selected_data, list) else selected_data
Output_root = Output_root+ "Obj_centric_temp2MLP_BERT_full" + str(Contrastive_temp)+"a"+str(Alpha)+"context"+str(Context_len) + dataset_tag + str(Data_percentage) + "/"
io.self_check_path_create(Output_root)
RESULT_FINISHED = 0
RESULT_TIMEOUT = 1
...
# DEFAULT_CONFIG = "videosaur_m/configs/videosaur/ytvis3_mixer7s.yml"
...
# Model_infer = model_infer_slot_att._Model_infer(parser.parse_args(),parser_s.parse_args(),GPU_mode,num_gpus,Using_contrast=True,Using_slot_bert=False,slot_ini= "rnn",gpu_selection=Gpu_selection,pooling="max",TPC=True)
# Model_infer = model_infer_slot_att._Model_infer(parser.parse_args(),parser_s.parse_args(),GPU_mode,num_gpus,slot_ini="rnn",Using_contrast=True,Using_slot_bert=True,Foundation_M="DINO",foundation_list  = ["DINO","SAM","MAE","CLIP"],fusion_method='moe_layer',gpu_selection=Gpu_selection,pooling="max",TPC=True)
Model_infer = model_infer_slot_att._Model_infer(parser.parse_args(),parser_s.parse_args(),GPU_mode,num_gpus,Using_contrast=False,Using_SP_regu = False,Using_SP = False,Using_slot_bert=True,slot_ini= "rnn",cTemp=Contrastive_temp,Foundation_M="DINO",foundation_list  = ["DINO","SAM","MAE","CLIP"],fusion_method='moe_layer',gpu_selection=Gpu_selection,alpha=Alpha,Context_len=Context_len, TPC=True)
...
# wrapped_model.eval()
# component_flops = get_component_flops(Model_infer)

# Print results
# print("\nFLOPs Breakdown (CPU Calculation):")
# print(f"Encoder: {component_flops['encoder'] / 1e9:.4f} GFLOPs")
# print(f"Processor: {component_flops['processor'] / 1e9:.4f} GFLOPs")
# print(f"Sliding Window: {component_flops['sliding_window_avg'] / 1e6:.2f} MFLOPs")
# print(f"Decoder: {component_flops['decoder'] / 1e9:.4f} GFLOPs")

# total_flops = sum(component_flops.values())
# print(f"\nTotal FLOPs: {total_flops / 1e9:.4f} GFLOPs")
...
if Continue_flag == False:
    ...
    # saved_log_cTemp = torch.load(Output_root + f"temp{ loadmodel_index }")
    # Model_infer.log_cTemp.data.copy_(saved_log_cTemp.to(Model_infer.log_cTemp.device))
...
features =None
visdom_id=0
# ---- forward-time CSV logger ----
# forward_csv_path = os.path.join(Output_root, "forward_time_log.csv")
forward_csv_path =  "forward_time_log.csv" 

header_needed = not os.path.exists(forward_csv_path)

forward_log_file = open(forward_csv_path, "a", newline="")
forward_writer = csv.writer(forward_log_file)
if header_needed:
    forward_writer.writerow(["epoch", "read_id", "time_iso", "forward_time_s"])
# ---------------------------------
while (1):
    ...
    print("forward time is :" + str(forward_time))
    ...
    if Evaluation == False and Evaluation_slots==False:
        Model_infer.optimization(labels_GPU,Enable_student,read_id=read_id) 
    

    # ---- write to CSV ----
    if Time_log == True:
        forward_writer.writerow([epoch,
                                read_id,
                                datetime.now().isoformat(),
                                forward_time])
        forward_log_file.flush()
    # ----------------------
    ...
    if Evaluation == False and Evaluation_slots == False:
        ...
        if read_id % 50== 0 and Visdom_flag == True  :
            ...
            plotter.plot('orth', 'orth', 'orth', visdom_id, Model_infer.lossDisplay_orth.cpu().detach().numpy())
    ...
    if (read_id % 1000) == 0  :
        ...
        torch.save(Model_infer.model.decoder.state_dict(), Output_root + "decoder" + str(saver_id) + ".pth")
        torch.save(Model_infer.model.temporal_binder.state_dict(), Output_root + "temporal_binder" + str(saver_id) + ".pth")
        torch.save(Model_infer.log_cTemp.detach().cpu(), Output_root + "temp" + str(saver_id) + ".pth")
    ...
    if dataLoader.all_read_flag ==1:
        ...
        if Save_feature_OLG: 
            break
        if epoch > Max_epoch  :
            output_file = eval_slots.process_metrics_from_excel(Output_root + "/metrics_video.csv", Output_root)
