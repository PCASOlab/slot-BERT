...
from videosaur_m.videosaur import configuration, losses, modules, optimizers, utils, visualizations
from videosaur_m.videosaur.data.transforms import Denormalize
from videosaur_m.videosaur.modules.presence_nn import Slot_Merger_Cosine_avg,Slot_Merger_Cosine
from model import model_operator
from math import sqrt
from feature_clip.feature import CLIPViT
from feature_mae.feature import MAEViT
from feature_sam.feature import SAMViT
from feature_dinov3.feature import DINOv3ViT
from model.slots.SPOT import SpatialPermutation
from feature_ensemble.feature_select import EncoderStack,FusionModule
...
from working_dir_root import Video_len, Evaluation_slots,Fintune,Evaluation
Use_SPOT = False
# Window_size=5
if Evaluation_slots:
    # from model.slots.temporal_slots_mask import Temporal_Binder
...
def build(
    ...
    Sim_threshold=0.9,
    Foundation_M = "CLIP",
    foundation_list: List[str] = ["DINO","SAM","MAE"],
    fusion_method="moe_layer",
    Context_len=5,
    ...
    if Foundation_M == "DINO": 
        encoder = modules.build_encoder(model_config.encoder, "FrameEncoder")
    elif Foundation_M == "CLIP":
        encoder = CLIPViT( )
    elif Foundation_M == "MAE":
        encoder = MAEViT()
    elif Foundation_M == "SAM":
        encoder = SAMViT()
    elif Foundation_M == "DINOv3":
        encoder = DINOv3ViT(model_type="vitb16")
    elif Foundation_M == "ensemble":
        # encoder_map = {
        #     "DINO": (modules.build_encoder, 768),
        #     "CLIP": (CLIPViT, 768),
        #     "MAE": (MAEViT, 768),
        #     "SAM": (SAMViT, 256)
        # }
        ...
        for model_name in foundation_list:
            ...
            # Add feature dimension tracking
            
            encoders.append(encoder)
            feature_dims.append(feature_dim)
        encoder = EncoderStack(encoders=encoders)

    if Foundation_M == "ensemble":
        # adapter = FusionModule(fusion_method="moe_layer")
        adapter = FusionModule(
            fusion_method=fusion_method, 
            feature_dims=feature_dims
        )
        # adapter = modules.MapOverTime(adapter)
    else:
        adapter = None
    ...
    if input_type == "image":
        ...
        if Foundation_M == "ensemble":
            # encoder = modules.MapOverTime2(encoder)
            pass
...
class ObjectCentricModel(pl.LightningModule):
    def __init__(
        ...
        *,
        ...
        self.temporal_binder = Temporal_Binder(slot_dim=self.initializer.dim, num_slots=7,num_frame=self.context_len)
        # self.future_state_prdt = future_state_predit(slot_dim=64, num_slots=7,num_frame=5)
        self.future_state_prdt = Temporal_Binder(slot_dim=self.initializer.dim, num_slots=7,num_frame=self.context_len)
    ...
    def forward_feature_stack(self, inputs: Dict[str, Any], Using_bert =False,slot_ini = "rnn", Mask_feat = False,img_sim = False,epoch=None,batch_files =None,Output_root=None) -> Dict[str, Any]:
        ...
        if self.adapter is not None:
            features_stack =  modules.Map_time_feature_stack(encoder_input,module1=self.encoder,epoch=epoch,batch_files =batch_files ,Output_root=Output_root)
        return features_stack
    
    def forward(self, inputs: Dict[str, Any],features_stack=None,feature_stack_s=None, Using_bert =False,slot_ini = "rnn", Mask_feat = False,img_sim = False,epoch=0,read_id=None,Output_root=None,Student= False) -> Dict[str, Any]:
        encoder_input = inputs[self.input_key]  # batch [x n_frames] x n_channels x height x width
        assert encoder_input.ndim == self.expected_input_dims
        batch_size = len(encoder_input)
        # with torch.no_grad():
        ...
        # encoder_output = self.encoder(encoder_input)
        if self.adapter is not None:
            # features_stack =  modules.Map_time_feature_stack(encoder_input,module1=self.encoder,epoch=epoch,read_id=read_id,Output_root=Output_root)
            encoder_output = modules.Map_time_adapt_reshape(encoder_input,outputs=features_stack, module2=self.adapter )
            # encoder_output_s = modules.Map_time_adapt_reshape(encoder_input,outputs=feature_stack_s, module2=self.adapter )

            
        else:
            encoder_output = self.encoder(encoder_input) # encoder_input.shape torch.Size([1, 24, 3, 224, 224])
        ...
        self.spatial_perm = SpatialPermutation(H=H, W=W, train_permutations='all', eval_permutations='standard')
        # Get permutation based on training mode
        if Evaluation_slots or Evaluation:
            Training= False
        else:
            Training= True
        if Use_SPOT and Evaluation == False and Evaluation_slots == False:
            permutation = self.spatial_perm.get_permutation(training= Training,Epoch=epoch)
            features = self.spatial_perm.apply_permutation(features, permutation)
        outputs = {
            "batch_size": batch_size,
            "encoder": encoder_output,
        }
        # with torch.no_grad():
        #     encoder_output["backbone_features"] = encoder_output_s["backbone_features"]
        if Student:
            with torch.no_grad():
                encoder_output["backbone_features"] = features
        outputs["targets"] = self.get_targets(inputs, outputs)
        # features = encoder_output["backbone_features"]
        ...
        # Pure_down_pool = nn.AvgPool3d((1,2,2),stride=(1,2,2))
         ...
         # slot initali shape : torch.Size([1, 7, 64])
        if slot_ini== "rnn":
        ...
        # slot shape slots.shape torch.Size([1, 24, 7, 64])
        if Using_bert:
            # slotst,slots = self.temporal_binder (slots)
            if (epoch%2)==0 or Evaluation_slots==True or Fintune ==True: # allow slots to be none bert reconstruction
                slots = temporal_slots_slide.apply_sliding_window_avg( self.temporal_binder,slots,window_size=self.context_len, stride=None)
                print("applying sbert")
            else:
                # slots = slots_op.align_permutations_across_n_frames(slots,n=1)
                print("not applying sbert")
        ...
        if Use_SPOT and Evaluation == False and Evaluation_slots == False:
            decoder_output["reconstruction"]  =  self.spatial_perm.inverse_permutation(decoder_output["reconstruction"], permutation)
            mask_permute  = self.spatial_perm.inverse_permutation(decoder_output["masks"].permute(0,1,3,2), permutation) 
            decoder_output["masks"] = mask_permute.permute(0,1,3,2)
