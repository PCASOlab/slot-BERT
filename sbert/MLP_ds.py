...
Output_root = Output_root+ "MLP_dynamic_slots_" + selected_data + str(Data_percentage) + "/"
...
DEFAULT_CONFIG = "videosaur_m/configs/videosaur/ytvis3_MLP_ds.yml"
...
Model_infer = model_infer_slot_att._Model_infer(parser.parse_args(),GPU_mode,num_gpus,Using_contrast=False,Using_SP_regu = True,Using_SP = True,Using_slot_bert=False,slot_ini= "rnn",gpu_selection=Gpu_selection,pooling="max",TPC=True)
# Model_infer = model_infer_slot_att._Model_infer(parser.parse_args(),GPU_mode,num_gpus,Using_contrast=False,Using_slot_bert=False,slot_ini= "rnn",cTemp=1.1,gpu_selection=Gpu_selection,pooling="max",TPC=True)
...
while (1):
    ...
    if (read_id % 1000) == 0  :
        ...
        torch.save(Model_infer.model.encoder.state_dict(), Output_root + "encoder" + str(saver_id) + ".pth")
        torch.save(Model_infer.model.processor.state_dict(), Output_root + "processor" + str(saver_id) + ".pth")
        torch.save(Model_infer.model.presence_nn.state_dict(), Output_root + "presence_nn" + str(saver_id) + ".pth")
        torch.save(Model_infer.model.decoder.state_dict(), Output_root + "decoder" + str(saver_id) + ".pth")
