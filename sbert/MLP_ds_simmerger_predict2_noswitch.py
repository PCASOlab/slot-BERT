...
Output_root = Output_root+ "0Merge_predict_sperate_noswitch" + dataset_tag + str(Data_percentage) + "/"
...
Model_infer = model_infer_slot_att._Model_infer(parser.parse_args(),GPU_mode,num_gpus,Using_contrast=False,Using_SP_regu = False,Using_SP = True,Using_slot_bert=True,slot_ini= "binder2+merger",gpu_selection=Gpu_selection,pooling="max",TPC=True)
...
if Continue_flag == False:
    ...
    # Model_infer.model.future_state_prdt.load_state_dict(torch.load(Output_root + 'temporal_binder' + loadmodel_index,map_location='cuda:0' ))
