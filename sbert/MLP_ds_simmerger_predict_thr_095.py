...
# Visdom_flag = False
# Display_flag = False
# loadmodel_index = '3.pth'Sim
Sim_threshold = 0.95
dataset_tag = "+".join(selected_data) if isinstance(selected_data, list) else selected_data
Output_root = Output_root+ "0Merge_predict" + str(Sim_threshold)+dataset_tag + str(Data_percentage) + "/"
...
Model_infer = model_infer_slot_att._Model_infer(parser.parse_args(),GPU_mode,num_gpus,Using_contrast=False,Using_SP_regu = False,Using_SP = True,Using_slot_bert=True,slot_ini= "binder+merger",Sim_threshold=Sim_threshold,gpu_selection=Gpu_selection,pooling="max",TPC=True)
